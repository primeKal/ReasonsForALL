...
class DBExtractor:
    ...
    def extract_schema(self, connection_string: str, rule_cap: int = 5, repo_url: str | None = None):
        """
        ...
        try:
            ...
            try:
                ...
                self.logger.warning(
                    f"DBExtractor: Connection failed with connect_args: {str(e)}")
                # If connection fails due to pooler rejecting the default_transaction_read_only parameter, retry without it
                if connect_args:
                    self.logger.warning(
                        "DBExtractor: Postgres pooler rejected startup options. Retrying without options...")
                    try:
                        engine = create_engine(connection_string, connect_args={
                                               "connect_timeout": 3})
                        metadata = MetaData()
                        self.logger.warning(
                            "DBExtractor: Reflecting 'public' schema without options...")
                        metadata.reflect(bind=engine, schema="public")
                    except SQLAlchemyError as retry_err:
                        self.logger.warning(
                            f"DBExtractor: Retry failed: {str(retry_err)}")
                        raise retry_err
                else:
                    raise e
            ...
            try:
                ...
                with engine.connect() as conn:
                    ...
                    try:
                        ...
                        triggers = [
                            {"name": r[0], "event": r[1], "table": r[2], "body": r[3][:500]} for r in trig_rows]
                    except Exception:
                        pass
                    ...
                    try:
                        ...
                        functions = [{"name": r[0], "type": r[1], "body": (
                            r[2] or "")[:500]} for r in func_rows]
                    except Exception:
                        pass
                ...
                self.logger.warning(
                    f"DBExtractor: Text-policy extraction skipped: {policy_err}")
            ...
            self.logger.error(
                f"DBExtractor: Failed to extract schema: {str(e)}")
            mocked_rules = [
                {"subject": "User", "predicate": "is_a", "object": "Class", "type": "ClassDefinition",
                    "quantifier": "none", "cardinality_value": None, "description": "Core User class."},
                {"subject": "Waiter", "predicate": "subClassOf", "object": "Employee", "type": "ClassHierarchy",
                    "quantifier": "none", "cardinality_value": None, "description": "Waiter ⊑ Employee subclass."},
                {"subject": "Employee", "predicate": "subClassOf", "object": "User", "type": "ClassHierarchy",
                    "quantifier": "none", "cardinality_value": None, "description": "Employee ⊑ User subclass."},
                {"subject": "Employee", "predicate": "has_Role", "object": "Role", "type": "ObjectProperty",
                    "quantifier": "min", "cardinality_value": 1, "description": "Employee ⊑ ≥1has_Role.Role"}
            ]
            return {
                "status": "simulated",
                ...
                "text_policies": [
                    {"title": "User Authentication Restriction",
                        "body": "Only authenticated users with verified identities are permitted to register actions or create records in the system.", "source_type": "inferred"},
                    {"title": "Employee Role Requirement",
                        "body": "An Employee is authorized to perform system actions only if they have been assigned at least one active, valid Role.", "source_type": "inferred"},
