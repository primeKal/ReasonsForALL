...
export default function LoginPage() {
  ...
  const handleGoogleLogin = async () => {
    const { error } = await supabase.auth.signInWithOAuth({
      provider: 'google',
      options: {
        redirectTo: `${window.location.origin}/dashboard/servers`
      }
    })
    ...
    <div className="min-h-screen flex items-center justify-center bg-background/90 p-4">
      <Card className="w-full max-w-md shadow-2xl bg-card border-border/50">
        <CardHeader className="space-y-1 text-center">
          <div className="w-12 h-12 mx-auto rounded-lg bg-primary flex items-center justify-center font-bold text-white text-xl shadow-lg mb-2">R</div>
        ...
        <CardContent className="space-y-4">
          ...
          <form onSubmit={handleEmailAuth} className="space-y-4">
            ...
            <Button type="submit" className="w-full h-11 text-white" disabled={loading}>
